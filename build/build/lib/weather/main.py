...
def start(message):
    ...
    :param message: Пользовательское сообщение с текстом команды
    :type message: messageObject
    
    :return: Задача добавления set_city(message), удаления del_st(message), изменения change_st(message) записи из расписания или подсказка о начале работы с ботом
...
def del_st(message):
    """Удаление прогноза из расписания (записи из БД)

    :param message: Пользовательское сообщение с текстом элемента расписания
    :type message: messageObject
...
def set_city(message):
    ...
    Выдаёт подсказку с просьбой ввести город, после чего вызывает get_city()

    :param message: Пользовательское сообщение с названием города
    :type message: messageObject

    :return: `True`, если вызов get_city() успешен, иначе `False`
    :rtype: bool
...
def get_city(message):
    ...
    :return: Сообщение об успехе или ошибке получения города
    :rtype: string
...
def set_time(message):
    ...
    после чего вызывает функцию получения времени get_time().

    :param message: Пользовательское сообщение со временем
    :type message: messageObject

    :return: `True`, если вызов get_time() успешен, иначе `False`
    :rtype: bool
...
def get_time(message):
    ...
    Принимает на вход пользовательское сообщение с информацией о времени,
    после чего вызывает функцию подтверждения keyboard_confirmation() или выводит сообщение о неверном вводе времени.
    
    :param message: Пользовательское сообщение с информацией о времени
    :type message: messageObject

    :return: Сообщение со временем или сообщение об ошибке ввода времени 
    :rtype: string
...
def keyboard_confirmation(message):
    ...
    :param message: Пользовательское сообщение с информацией об отправителе
    :type message: messageObject

    :return: `True`, если вызов keyboard_confirmation() успешен, иначе `False`
    :rtype: bool
...
def wrong_city(message):
    ...
    Принимает на вход сообщение с информацией об отправителе,
    после чего передаёт обработчику информацию о вызове функции получения города get_city() 
    и выводит сообщение пользователю.

    :param message: Пользовательское сообщение с информацией об отправителе
    :type message: messageObject

    :return: Сообщение об ошибке ввода города
    :rtype: string
...
def wrong_time(message):
    ...
    после чего передаёт обработчику информацию о вызове функции получения времени get_time().

    :param message: Пользовательское сообщение с информацией об отправителе
    :type message: messageObject

    :return: Сообщение об ошибке ввода времени
    :rtype: string
...
def get_weather(city):
    ...
    :return: Кортеж данных о погоде (осадки, температура)
    :rtype: tuple
...
def abb_to_city(message):
    ...
    :param message: Сокращение города
    :type message: string
...
def callback_worker(call):
    ...
    :param call: Выбор `yes`/`no` пользователя
    :type call: string

    :return: Информация о добавлении в базу данных или подсказка о начале работы с ботом
    :rtype: string
...
def mail():
    ...
    то получает прогноз погоды, а затем отправляет соответствующему пользователю.

    :return: Сообщение пользователю с прогнозом погоды
    :rtype: messageObject
    """
